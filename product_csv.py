""" 
This converting DB into CSV row
...
csv_fields = ["name","level","published","created_on","review_count","avg_rating"]
...
for product, review_count, avg_rating in (
    session.query( Product,reviews_statement.c.review_count, reviews_statement.c.avg_rating)
    .outerjoin(reviews_statement, Product.id == reviews_statement.c.product_id).limit(6)
    ):
    csv_writer.writerow(
       {
            "name": product.name,
